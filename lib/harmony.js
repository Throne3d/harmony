...
class Harmony {
  ...
  bindEvents() {
    ...
    client.on('warn', this.handleWarning.bind(this));
  }
  ...
  performReactions(message, reactions) {
    ...
    promisedResponse = promisedResponse
      ...
      .catch(error => {
        ...
        let destroyReacts = reacts.map(x => x.remove);
        return Promise.all(destroyReacts).then(_ => { throw(error) });
      });
  ...
  performReactionsWithFallback(message, reactions, fallback) {
    let promisedResponse = performReactions(message, reactions)
      .catch(reacted => message.reply(fallback));
    return promisedResponse;
  }
  ...
  longRespondTo(message, text, summary) {
    ...
    const truncatedString = _.truncate(text, { length: cutoffBoundary, omission: '…' });
    let truncated = truncatedString !== text;
    if (truncated) {
      warning = "that got a *really* long response!";
    }

    const splits = _.chunk(truncatedString, splitLength).map(x => x.reduce((y, z) => y + z));
    console.log(splits);
