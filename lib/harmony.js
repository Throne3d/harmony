...
class Harmony {
  ...
  // if it fails, says the fallback string
  performReactionsWithFallback(message, reactions, fallback) {
    let promisedResponse = Promise.resolve(true);
    // use this instead of Promise.all so the reactions are ordered:
    for (const reaction of reactions) {
      promisedResponse = promisedResponse.then(_ => message.react(reaction));
    }
  ...
  performReactionWithFallback(message, reaction, fallback) {
    return performReactionsWithFallback(message, [reaction], fallback);
  }

  // gives a potentially long response to a message
  // accepts a (short!) summary to post with the potential warning message.
  longRespondTo(message, text) {
    if (text.length > 3900) { /* 3900 in case the boundary isn't exactly 2000 per message */
      // Truncate it.
      return Promise.all([
        message.author.send(text.substring(0, 1950))
          .then(_ => message.author.send(text.substring(1950, 3901))),
        this.performReactionsWithFallback(message, ["❗", "☁"], "that got a *really* long response! I've truncated it and sent you the start.")
      ]);
    }
    if (text.length > 1000) {
      // PM it.
      return Promise.all([
        ...
        this.performReactionWithFallback(message, "📬", "that got a long response! I've sent you a PM with it.")
      ]);
    }
    ...
    this.commands = {
      help: {
        ...
        process: (command, message) => {
          ...
          for (const commandName of Object.keys(commands)) {
            ...
            help += `\`${commandName}\`: ${command.description}`;
            if (command.aliases && command.aliases.length > 0) {
              let aliasList = command.aliases.map(x => "`" + x + "`").join(", ");
      ...
      dice: {
        ...
        process: (command, message, argString) => {
          ...
          return this.longRespondTo(message, reply);
        },
      },
    };
  ...
  processCommand(command, message) {
    ...
    for (const candidateName of Object.keys(commands)) {
      ...
      triedNames.find(trial => {
        var regex = new RegExp(`^${_.escapeRegExp(trial)}(\\s|$)`);
        console.log("Testing", command, "against", regex);
