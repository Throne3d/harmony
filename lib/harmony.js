...
class Harmony {
  ...
  // promises to respond to the self mention
  processSelfMention(message) {
    log.debug("Self-mentioning message:", this.debugifyMessage(message));
  ...
  processMessage(message) {
    ...
    var promise = this.checkMessageMentionsMe(message)
      .then(mentionsMe => {
        if (mentionsMe) return this.processSelfMention(message);
